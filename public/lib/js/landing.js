...
const landingAnimation = () => {
  let flObj = '#fallingDevices',
      ...
      scene = [flObj, shdws],
      signs = '#signpost';
  tl_master = new TimelineMax({
    id: 'master'
  });
  tl_landingSequence = new TimelineMax({
    id: 'landingSequence'
  });
  tl_signRotator = new TimelineMax({
    id: 'signage'
  });
  tl_master.add([tl_landingSequence.add([TweenMax.to(flObj, 1.5, {
    delay: 0,
    bottom: "13vh",
    rotation: 0,
    ease: Bounce.easeOut
    ...
    marginLeft: 0,
    ease: Back.easeIn
  }), TweenMax.to(scene, 0.01, {
    delay: 1.5,
    clearProps: 'all'
  })]), tl_signRotator.add([TweenMax.to(signs, 0.5, {
    delay: 1.75,
    rotation: '-=180',
    ease: Bounce.easeOut
